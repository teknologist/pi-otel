...
import { ATTR_AGENT_NAME, ATTR_FINISH_REASONS, ATTR_HTTP_STATUS_CODE, ATTR_PI_CWD, ATTR_PI_SESSION_ID, ATTR_PROVIDER_NAME, ATTR_REQUEST_MODEL, ATTR_RESPONSE_ID, ATTR_RESPONSE_MODEL, ATTR_SYSTEM, applyUsageAttrs, GEN_AI_SYSTEM_PI, } from "./attrs.js";
...
function modelIdFromContext(ctx) {
    return firstString(ctx?.model?.id);
}
function providerFromContext(ctx) {
    return firstString(ctx?.model?.provider);
}
...
function providerFromPayload(event, payload) {
    const e = event;
    const p = payload && typeof payload === "object"
        ? payload
        : undefined;
    return firstString(e?.provider, e?.providerName, e?.modelProvider, p?.provider, p?.providerName, p?.modelProvider, p?.provider?.name);
}
export default function (pi) {
    ...
    pi.on("before_provider_request", async (event, ctx) => {
        ...
        const model = modelFromPayload(payload) ??
            ...
            firstString(currentModelRef?.id);
        const provider = providerFromContext(ctx) ??
            firstString(currentModelRef?.provider) ??
        ...
        if (provider)
            attrs[ATTR_PROVIDER_NAME] = provider;
